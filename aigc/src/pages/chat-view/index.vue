...
<template>
  <view class="menu-page">
    ...
    <view class="menu-style">
      <text
        ...
        @click="selectTar(index)"
        :key="index"
        :class="{ 'select-menu': selectIndex.navIndex === index }"
        >{{ item }}</text
      >
    </view>
  </view>
  ...
  <chatArea
    class="chatAreaVue"
    v-show="selectIndex.navIndex === 1 && aiChat.messages?.length > 0"
  />
  <!-- AI绘画 -->
  ...
  <!-- 个人中心 -->
  <personCenter v-show="selectIndex.navIndex === 0" />
  <!-- 登录页 -->
